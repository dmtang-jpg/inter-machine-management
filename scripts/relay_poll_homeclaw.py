#!/usr/bin/env python3
"""Agent Relay poller for homeclaw — cron-friendly, silent when no new messages."""
import json, os, sys
...
STATE_FILE = os.path.expanduser(f"~/.hermes/relay_{AGENT}_state.json")
...
def api(path, method="GET", data=None):
    ...
    try:
        ...
        print(f"[relay_poll] API error: {e}", file=sys.stderr)
        return {"error": str(e)}
...
def main():
    ...
    print(f"📬 homeclaw relay — {len(new_msgs)} 条新消息:")
    for m in new_msgs:
